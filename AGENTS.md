...
本仓库是一个名为 `rohand` 的 ROS 2 `ament_python` 软件包。

- `scripts/roh_ap001_node/` 包含 AP001 灵巧手的 Modbus、串口和 CAN 可执行节点。
- `common/` 包含共享寄存器映射，以及不同版本的串口/CAN 协议实现。
- `test/` 包含版权、Flake8 和 PEP 257 的 ROS 软件包代码检查测试。
- `resource/`、`package.xml`、`setup.py` 和 `setup.cfg` 定义软件包元数据及安装行为。
